...
function mt:set_art(art)
	if not self.owner or not jass.GetPlayerAlliance(self.owner:get_owner().handle, ac.player.self.handle, jass.ConvertAllianceType(6)) then
		return
	end
	if not self:is_visible() then
		return
...
-- 格式化字符串
 ...
 -- [技能等级]
 -- [需要扩展等级数据]
 --modify by jeff 20190122  %%([%w_]*)%%   →  %%([%S_]*)%%
local function format_string(self, str, hero, level, need_level)
	return str:gsub('%%([%S_]*)%%', function(name)
		local v = self[name]
...
function unit.__index:add_skill(name, type, slotid, data)
	...
	if type == '物品' then
		japi.EXSetAbilityDataReal(skill:get_handle(), 1, 0x69, skill.cool or 0)
	end	
	...
	--如果是物品则不执行，否则会触发两次on_add事件
	if type ~= '物品' and type ~= '神符' then
