...
function player.__index:Map_CommentTotalCount()
    local handle = self.handle
    if global_test then 
        return self.total_comment or 130500
    else    
        return 130500
...
function player.__index:Map_AddServerValue(key,value,f)
    ...
    local value = self:Map_GetServerValue(key) + tonumber(value)
    self:Map_SaveServerValue(key,value)
end
...
if global_test then 
    function player.__index:Map_GetMapLevel()
        if ac.flag_use_mall then 
            return (self.map_level or 5) + (self['局内地图等级'] or 0)
        else     
            return 1
        end    
    end
