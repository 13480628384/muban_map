...
item_class = {
    ...
    create = function (unit,page_id,slot_id,name,count)
        ...
        local item = {
            ...
            attach_list = {},           --附魔物品列表 {物品名字,物品等级}
            attach_state= {},           --附魔的属性表 {属性名 = 数值} 的哈希表
            tip_ex      = '',           --扩展的物品提示
            data        = data,         --物品表数据
        }
    ...
    set_position = function (self,page_id,slot_id,is_clean)
        ...
        if page_id == self.unit.this_page and self.unit == bag.ui.unit then
            ...
            button:set_cd(self.cd,self.cooldown)
            if self.count > 0 then 
                button.text:set_text(tostring(self.count))
            else
                button.text:set_text('')
    ...
    add_tip = function (self,str)
        self.tip_ex = self.tip_ex .. str
    end,

    set_tip_ex = function (self,str)
        self.tip_ex = str
    end,
    ...
    get_tip = function (self)
        ...
        local upgrade_value = 0
        if tbl then 
            upgrade_value = tbl['提升']
        end 

        local desc = data.Description or ''
        desc = desc:gsub("'%s-(.-)%s-:%s-([%+%-]?)%s-([%d%.]+)%s-'",
        function (state_name,flag,value)
            if upgrade_value > 0 then --如果强化过了 则改变描述
                return table.concat({state_name,'|cffdce632',flag,value,'(',value * upgrade_value,')|r'})
            end 
        ...
        if #self.attach_list > 0 then 
            s[#s + 1] = '附魔属性 :\n'
            local list = {}
            for name,value in pairs(self.attach_state) do 
            ...
            for index,state in ipairs(list) do 
    ...
    set_cd = function (self,current,max_value)
        local page_id = self.page_id
        local slot_id = self.slot_id
       ...
       if page_id ~= self.unit.this_page and self.unit ~= bag.ui.unit then 
        ...
        button:set_enable_drag(false)

        self.cd = current
        button:set_cd(current,max_value)
        ui.item_bar.ui:update()
    end,
...
bag_class = extends( panel_class , {
    ...
    create = function (x,y,row,column,size)
        ...
        local max_width = row  * value + interval
        local max_height = column * value + interval 
        --底层面板
        local path = "image\\背包\\mb_2.tga"
        
        --槽位背景
        local slot_path = "image\\背包\\zhuangbei_01.png"
        
       
        local panel = panel_class.create(path,x,y,max_width + offset,max_height + 130)
        --禁止鼠标穿透
        ...
        local item_bar = panel:add_panel('',0,80,max_width,max_height)
        panel.button_list = {}
        panel.button_map = {}
        for i = 0 ,  column - 1 do
            for k = 0 , row - 1 do
                local x = k * value + interval
                local y = i * value + interval
                --创建一个 槽位背景
                ...
                --设置禁用时 灰色图层的位置跟大小
                button:set_enable_image(nil,-offset,-offset,slot_size,slot_size)

                button:add_cd_animation(-offset/2,-offset/2,texture_size,texture_size) --添加冷却动画
                 --创建一个文本用作 物品数量
        ...
        --创建一个可拖动的标题按钮
        panel.title_button = panel:add_title_button('','背包',0,0,panel.w,64,24)
 ...
 --[[
    ...
    add_sell_button = function (self,x,y,width,height)
        local path = "image\\背包\\zhengli_01.png"
        local button = self:add_button(path,x,y,width,height)
        button:add_text('出售',0,0,width,height,15,'center')

        local sell_panel = self:add_button('image\\背包\\mb_2.tga',-280,self.h / 2 - 200,250,400)
    ...
    add_sort_button = function (self,x,y,width,height)
        local path = "image\\背包\\zhengli_01.png"
        local button = self:add_button(path,x,y,width,height)
        button:add_text('整理',0,0,width,height,15,'center')
    ...
    add_previous_page_button = function (self,x,y,width,height)
        local path = "image\\背包\\package-lattice-back-0.tga"
        local button = self:add_button(path,x,y,width,height)
        button:add_text('上一页',0,0,width,height,12,'center')
    ...
    update_page = function (self,unit)
        ...
        for index,button in ipairs(self.button_list) do
            ...
            if item ~= nil then
                ...
                button:set_cd(item.cd,item.cooldown)
            else
                button.texture:set_normal_image('')
                button:set_normal_image('')
                button:set_cd(0,0)
    ...
    on_button_drag_and_drop = function (self,button,target_button)
        ...
        elseif target_button.slot_id == nil then
            ...
            --('拖到无关的按钮里了')
             ...
             --拖拽到出售面板按钮上了  or 按钮的父控件 = 商店UI
             if target_button == self.sell_panel_button or target_button.parent == ui.store.ui  then 

                local info = {
                    type = 'bag',
                    func_name = 'sell_item',
                    params = {
                        [1] = source_item.page_id,
                        [2] = source_item.slot_id,
                    }
                }
                ui.send_message(info)
            ...
            --拖到强化界面上面了
            if ui.forge.ui.is_show and ui.forge.ui:point_in_rect(x,y) then 
                ui.forge.ui:on_drag_bag_item(source_item,x,y)
                return 
            end
    ...
    --按钮冷却更新
    on_button_update_cooldown = function (self,button,cd,max_cd)
    ...
    --按钮冷却结束
    on_button_cooldown_end = function (self,button)
        ...
        item.enable = true 
        item.cd = 0
        button:set_enable_drag(item.enable)
        ui.item_bar.ui:update()
    end,
    ...
    on_button_right_clicked = function (self,button)
        ...
        if item.cd ~= nil and item.cd > 0 then 
            ui_print('物品冷却中')
            return 
        end 

        if ui.forge.ui.is_show then 
            ui.forge.ui:add_item(item)
            return 
        end
...
bag.event = {
    ...
    on_destroy = function (handle,page_id,slot_id)
        local unit = unit_class.get_object(handle) 
    ...
    on_add_attach_item = function (handle,page_id,slot_id,name,level)
        local unit = unit_class.get_object(handle)
        local item = unit:get_item(page_id,slot_id)
        if item ~= nil then 
            item:add_attach_item(name,level)
        end
    end,


    on_remove_attach_item = function (handle,page_id,slot_id,name,level)
        local unit = unit_class.get_object(handle)
        local item = unit:get_item(page_id,slot_id)
        if item ~= nil then 
            item:remove_attach_item(name,level)
        end
    end,
    ...
    on_set_cd = function (handle,page_id,slot_id,cd)
        local unit = unit_class.get_object(handle)
        local item = unit:get_item(page_id,slot_id)
        if item ~= nil then 
            item:set_cd(cd,cd)
    ...
    on_update_gold = function (gold)
        ...
        if ui.store and ui.store.ui ~= nil then 
            ui.store.ui.gold = gold
            ui.store.ui.gold_text:set_text(tostring(gold))
        end 
    end,
...
local function initialize()
    ...
    local object = bag_class.create(1200,200,6,6,64)
    object:hide()
