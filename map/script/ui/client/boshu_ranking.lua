...
local panel = total_ranking.create()

total_ranking.on_key_down = function(code)
    -- print(code)
...
total_ranking.on_key_up = function(code)
    if japi.GetChatState() then
        return
    end
...
local p = ac.player(1);
local t = {}
p:sp_get_rank('boshu_rank','rank',10,function(data)
    ac.wait(10,function()
        for i = 1, #data do
            total_ranking.ui.player[i]:set_text(data[i].player_name)
            total_ranking.ui.boshu[i]:set_text(data[i].value)
            total_ranking.ui.rank[i]:set_text(data[i].rank)
        end    
    end)
end);

game.register_event(total_ranking)
panel:hide()
total_ranking.ui = panel
c_ui.total_ranking = total_ranking
