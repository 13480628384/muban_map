...
local today_gold_ranking = {}

today_gold_ranking = extends( panel_class,{
    create = function()
        --创建底层面板 x,y,width,height
        local panel = panel_class.create('image\\提示框\\bj2.tga',470,200,475,620)
        --panel:set_alpha(0xff*0.9)
        --继承
        extends(today_gold_ranking,panel)
        
        --头部
        local top_panel = panel:add_panel('',0,20,475,100)
        --标题 (title,0,0,width,height,font_size,4)
        local title = top_panel:add_text('巅峰排行榜',0,0,475,100,16,4)
        title:set_color(243,246,4,1)
        panel.title = title
        --小标题 显示难度
        local two_title = top_panel:add_text('(今日-圣人模式)',0,0,475,25,10,4)
        ...
        --日期
        local date = top_panel:add_text('每日0点奖励皮肤碎片',280,top_panel.h-35,200,25,8,4)
...
local panel = today_gold_ranking.create()

today_gold_ranking.on_key_down = function(code)
...
today_gold_ranking.on_key_up = function(code)
    if japi.GetChatState() then
        return
    end
...
local p = ac.player(1);
local t = {}
p:sp_get_rank('today_boshu','rank',10,function(data)
    ac.wait(10,function()
        for i = 1, #data do
            today_gold_ranking.ui.player[i]:set_text(data[i].player_name)
            today_gold_ranking.ui.boshu[i]:set_text(data[i].value)
            today_gold_ranking.ui.rank[i]:set_text(data[i].rank)
        end    
    end)
end);

game.register_event(today_gold_ranking)
panel:hide()
today_gold_ranking.ui = panel
c_ui.today_gold_ranking = today_gold_ranking
return today_gold_ranking